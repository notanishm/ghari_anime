...
router.post('/', (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM downloads WHERE episode_id = ?').get(episode_id);
  if (existing) return res.status(409).json({ error: 'Episode already in downloads' });

  const result = db.prepare('INSERT INTO downloads (anime_id, episode_id, url) VALUES (?, ?, ?)')
    .run(anime_id, episode_id, url);
